...
const AdminMissions = () => {
  ...
  const managerIdNum = toNumberId(manager?.id) || 0;

  // --- RTK QUERY HOOKS ---
  const { data: missions = [], isLoading: missionsLoading } = useGetMissionsByManagerQuery(managerIdNum, {
    skip: !managerIdNum,
    pollingInterval: 15000 // רענון אוטומטי
  });
  ...
  const [summarizeAI, { isLoading: isGenerating }] = useSummarizeMissionAIMutation();
  ...
  const availableAgents = agentsData.filter((a: Agent) => a.status === 'AVAILABLE');
  // -------------------------
  ...
  const activeMissions = useMemo(
    () => missions.filter((m: Mission) => m.status === 'IN_PROGRESS' || m.status === 'PENDING'),
    [missions]
  );

  const completedMissions = useMemo(
    () => missions.filter((m: Mission) => m.status === 'COMPLETED' || m.status === 'ABORTED'),
    [missions]
  );
  ...
  useEffect(() => {
    ...
    const isActive = selectedMission?.status === 'IN_PROGRESS' || selectedMission?.status === 'PENDING';
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      await summarizeAI(missionIdNum).unwrap();
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const triggerCompleteMission = () => {
    ...
    const missionIdNum = toNumberId(selectedMission.id);
    if (missionIdNum !== null) setShowCompleteConfirm(missionIdNum);
  };

  const confirmCompleteMission = async () => {
    if (showCompleteConfirm === null) return;
    try {
      await completeMission(showCompleteConfirm).unwrap();
      if (missionsView === 'ACTIVE') setSelectedMission(null);
    } catch (err) {
      console.error(err);
    } finally {
      setShowCompleteConfirm(null);
  ...
  const triggerDeleteMission = () => {
    ...
    const missionIdNum = toNumberId(selectedMission.id);
    if (missionIdNum !== null) setShowDeleteConfirm(missionIdNum);
  };

  const confirmDeleteMission = async () => {
    if (showDeleteConfirm === null) return;
    try {
      await deleteMission(showDeleteConfirm).unwrap();
  ...
  const isMissionActive = selectedMission?.status === 'IN_PROGRESS' || selectedMission?.status === 'PENDING';
  ...
  return (
    <AdminLayout>
      ...
      <div className="h-[calc(100vh-60px)] flex flex-col font-mono text-emerald-500 bg-[#010504] relative p-4 overflow-hidden" dir="ltr">
        ...
        <div className="flex-1 min-h-0 grid grid-cols-12 gap-4 overflow-hidden">
          <aside className="col-span-3 bg-[#010806]/80 border border-emerald-900/40 rounded p-3 flex flex-col gap-2 overflow-hidden shadow-2xl backdrop-blur-sm">
            ...
            <h2 className="text-xs font-black tracking-widest text-emerald-100 uppercase flex items-center gap-2 border-b border-emerald-500/30 pb-2 mt-2">
              {missionsView === 'ACTIVE' ? (
                <><Radio className="animate-pulse text-emerald-400" size={14} />Active Missions</>
              ) : (
                <><CheckCircle size={14} />Inactive Missions</>
              )}
            </h2>
              ...
              <>
                ...
                <div className="border border-emerald-900/50 rounded p-3 bg-black/30 mb-3 shrink-0">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-xs font-black text-emerald-400 uppercase tracking-widest">Mission Description</h4>
                    <button onClick={() => setShowDescModal(true)} className="text-[10px] text-emerald-500 uppercase font-bold flex items-center gap-1 hover:text-emerald-300">
                ...
                <div className="border border-emerald-900/50 rounded p-3 bg-black/20 mb-4 shrink-0">
                  <h4 className="text-xs font-black text-emerald-400 uppercase tracking-widest mb-2">Assigned Agents</h4>
                  {selectedMission.assignedAgents && selectedMission.assignedAgents.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {selectedMission.assignedAgents.map((agent) => (
                        <div key={agent.id} className="px-2.5 py-1.5 rounded border border-emerald-800/60 bg-[#03140f] text-emerald-200 text-xs">
                          <span className="font-black">{agent.codename}</span>
                          {agent.specialty ? <span className="text-emerald-500"> • {agent.specialty}</span> : null}
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="text-xs text-emerald-700 uppercase tracking-widest">No agents assigned</div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-12 gap-4 flex-1 min-h-0">
                  <div className="col-span-5 border border-emerald-500/70 rounded p-5 h-full flex flex-col bg-[#03140f]/60 shadow-[0_0_24px_rgba(16,185,129,0.15)] overflow-hidden">
                    <div className="flex items-center justify-between mb-2 border-b border-emerald-700/40 pb-2 shrink-0">
                      ...
                      <button onClick={() => setShowReportsModal(true)} className="text-[10px] text-emerald-500 uppercase font-bold flex items-center gap-1 hover:text-emerald-300">
                        <Maximize2 size={12} /> Expand
                      </button>
                    </div>
                    ...
                    <div className="flex-1 min-h-0 overflow-hidden flex flex-col justify-start">
                      {missionReports.slice(0, 1).map((r, i) => (
                        <div key={r.id || i} className="bg-[#022018] border border-emerald-700/60 border-l-4 border-l-emerald-400 p-4 rounded">
                      ...
                      {missionReports.length === 0 && (
                        <div className="text-xs text-emerald-700 uppercase tracking-widest py-6 text-center my-auto">No reports for this mission</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="col-span-7 border border-amber-600/40 rounded p-5 h-full flex flex-col bg-[#140f03]/40 shadow-[0_0_24px_rgba(245,158,11,0.15)] overflow-hidden">
                    <div className="flex items-center justify-between mb-2 border-b border-amber-700/30 pb-2 shrink-0">
                      ...
                      <button onClick={() => setShowAiModal(true)} className="text-[10px] text-amber-300 uppercase font-bold flex items-center gap-1 hover:text-amber-100">
                        <Maximize2 size={12} /> Expand
                      </button>
                    </div>
        ...
        {/* מודלים למיניהם (ללא שינוי מהותי) */}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-[#0a0202]/70 backdrop-blur-md border border-red-600/80 w-full max-w-md p-6 shadow-[0_0_50px_rgba(220,38,38,0.3)] font-mono animate-in zoom-in-95 duration-200">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-sm text-red-200/90 mb-8 uppercase tracking-wider font-bold">Are you absolutely sure you want to end this operation?</p>
                <div className="flex gap-4 w-full">
                  <button onClick={() => setShowCompleteConfirm(null)} className="flex-1 bg-transparent border border-red-900/50 text-red-400 py-3 uppercase font-black text-xs hover:bg-red-950/50">CANCEL</button>
                  <button onClick={confirmCompleteMission} className="flex-1 bg-red-600/90 text-white py-3 uppercase font-black text-xs hover:bg-red-500">CONFIRM</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {showDeleteConfirm !== null && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-[#100303]/80 backdrop-blur-md border border-red-800 w-full max-w-md p-6 font-mono animate-in zoom-in-95">
              <div className="flex flex-col items-center text-center">
                <Trash2 size={32} className="text-red-600 mb-4 animate-pulse" />
                <h2 className="text-xl font-black text-red-600 uppercase mb-2">PERMANENT DELETION</h2>
                <p className="text-xs text-red-300/80 mb-8 font-bold">This action cannot be reversed.</p>
                <div className="flex gap-4 w-full">
                  <button onClick={() => setShowDeleteConfirm(null)} className="flex-1 bg-transparent border border-red-900/50 text-red-400 py-3 font-black text-xs">ABORT</button>
                  <button onClick={confirmDeleteMission} className="flex-1 bg-red-700/90 text-black py-3 font-black text-xs hover:bg-red-500">PURGE RECORD</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {showDescModal && selectedMission && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-[#010a08] border border-emerald-600/60 w-full max-w-3xl p-6">
              <h2 className="text-lg font-black text-emerald-300 uppercase mb-3">Mission Description</h2>
              <p className="text-sm text-emerald-100 whitespace-pre-wrap leading-6">{selectedMission.description}</p>
              <button onClick={() => setShowDescModal(false)} className="mt-5 px-4 py-2 bg-emerald-600 text-black font-black uppercase text-xs">Close</button>
            </div>
          </div>
        )}
        ...
        {showReportsModal && selectedMission && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-[#010a08] border border-emerald-600/60 w-full max-w-3xl p-6 max-h-[80vh] flex flex-col">
              <h2 className="text-lg font-black text-emerald-300 uppercase mb-3 shrink-0">Mission Reports</h2>
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3">
                {missionReports.map((r, i) => (
                  <div key={r.id || i} className="bg-[#022018] border-l-4 border-l-emerald-400 p-4 rounded">
                    <div className="flex justify-between text-xs text-emerald-400 mb-1">
                      <span className="font-black">[{r.author?.codename || r.author?.fullName || 'UNKNOWN'}]</span>
                      <span>{new Date(r.timestamp || r.createdAt || 0).toLocaleString('he-IL')}</span>
                    </div>
                    <p className="text-sm text-emerald-100 whitespace-pre-wrap">{r.rawText || r.text}</p>
                  </div>
                ))}
              </div>
              <button onClick={() => setShowReportsModal(false)} className="mt-5 px-4 py-2 bg-emerald-600 text-black font-black text-xs self-end">Close</button>
            </div>
          </div>
        )}
        ...
        {showAiModal && selectedMission && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-[#010a08] border border-emerald-600/60 w-full max-w-3xl p-6">
              <h2 className="text-lg font-black text-emerald-300 uppercase mb-3">AI Brief</h2>
              <p className="text-sm text-amber-100 whitespace-pre-wrap leading-6">{selectedMission.aiIntelligenceSummary}</p>
              <button onClick={() => setShowAiModal(false)} className="mt-5 px-4 py-2 bg-emerald-600 text-black font-black text-xs self-end">Close</button>
            </div>
          </div>
        )}
...
const CreateMissionModal = ({ availableAgents, onClose }: any) => {
  const { user: manager } = useAuth();
  const [createMission] = useCreateMissionMutation(); // שימוש במוטציה של רידקס
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createMission(formData).unwrap(); // שולח דרך הרידקס
    } catch (err) {
      console.error(err);
    }
  ...
  if (modalStatus === 'scanning') {
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" dir="ltr">
        ...
        <div className="relative bg-[#010a08] border-2 border-emerald-500 w-full max-w-md p-8 font-mono text-center flex flex-col items-center justify-center gap-6">
  ...
  if (modalStatus === 'success') {
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" dir="ltr">
        <div className="absolute inset-0 bg-black/90 backdrop-blur-md" />
        <div className="relative bg-[#010a08] border-2 border-emerald-400 w-full max-w-md p-8 font-mono text-center flex flex-col items-center justify-center gap-6 animate-in zoom-in-95">
          ...
          <button onClick={onClose} className="w-full bg-emerald-600 text-black py-3 font-black uppercase text-xs">RETURN TO CONTROL</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" dir="ltr">
      ...
      <div className="relative bg-[#010a08] border-2 border-emerald-600/60 w-full max-w-2xl font-mono flex flex-col">
        <div className="bg-emerald-950/80 border-b border-emerald-600/60 px-6 py-4 flex justify-between items-center">
          <h2 className="text-xl font-black text-emerald-300 uppercase">DRAFT NEW OPERATION</h2>
          <button onClick={onClose} className="text-emerald-500 hover:text-red-400"><X size={24} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="space-y-2">
            <label className="text-xs text-emerald-400 block uppercase font-bold">OPERATION CODENAME</label>
            <input required type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} className="w-full bg-black/60 border border-emerald-700/60 p-3 text-sm text-emerald-100 outline-none" />
          </div>
          <div className="space-y-2">
            <label className="text-xs text-emerald-400 block uppercase font-bold">MISSION DIRECTIVES</label>
            <textarea required rows={4} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full bg-black/60 border border-emerald-700/60 p-3 text-sm text-emerald-100 outline-none" />
          </div>
          <div className="space-y-2">
            <label className="text-xs text-emerald-400 block uppercase font-bold border-b border-emerald-900/50 pb-2 mb-2">ASSIGN PERSONNEL</label>
            <div className="max-h-40 overflow-y-auto custom-scrollbar border border-emerald-900/50 p-2 bg-black/40">
              {availableAgents.length > 0 ? availableAgents.map((agent: any) => (
                  <div key={agent.id} onClick={() => handleToggleAgent(agent)} className={`p-2 mb-1 flex justify-between items-center cursor-pointer border rounded-sm text-sm font-bold uppercase ${formData.assignedAgents.find((a: any) => a.id === agent.id) ? "bg-emerald-600/30 border-emerald-500 text-emerald-200" : "bg-[#020d0a] border-emerald-900/60 text-emerald-600"}`}>
                    <span>{agent.codename}</span><span className="text-[10px]">{agent.specialty}</span>
                  </div>
                )) : <div className="text-center text-xs font-bold text-red-500 p-4">NO AVAILABLE AGENTS</div>}
            </div>
          </div>
          <div className="flex justify-end gap-4 pt-6 border-t border-emerald-900/60">
            <button type="button" onClick={onClose} className="px-6 py-3 text-sm font-bold text-emerald-600">CANCEL</button>
            <button type="submit" disabled={formData.assignedAgents.length === 0} className="bg-emerald-600 text-black px-8 py-3 text-sm font-black disabled:opacity-50">LAUNCH MISSION</button>
          </div>
        </form>
      </div>
    </div>
